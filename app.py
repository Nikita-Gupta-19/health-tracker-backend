...
# On Linux (Render/Docker), it will use the system 'tesseract' command automatically
...
def predict():
    ...
    try:
        ...
        # Return confidence score if model supports predict_proba
        confidence = None
        try:
...
def chat():
    """
    Handle general health questions using Gemini AI
    Expected input JSON:
    {
    ...
    try:
        ...
        # Prompt for Gemini with medical disclaimer context
        prompt = f"""
...
INSTRUCTIONS:
1. Provide accurate, helpful, and clear information based on medical knowledge.
2. Use a friendly and supportive tone.
3. Use bullet points for readability when appropriate.
4. IMPORTANT: Always include a brief disclaimer at the end stating that you are an AI and the user should consult a professional doctor for medical advice.
5. Do NOT provide specific dosages for medications.
6. If the question is not health-related, politely redirect the user.

Format your response in plain text or simple markdown (avoiding complex blocks).
"""
        ...
        return jsonify({
            "response": result_container["text"]
        })
